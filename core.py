"""
核心统计函数：表头检测、数据合并、预处理、子组统计、控制限、判异、能力分析
"""
import pandas as pd
import numpy as np
...
def process_data(files, header_rows, mapping_config):
    ...
    # 处理规格限（如果是来自列，则取该列第一个有效值作为常数）
    def _extract_spec(col_name):
        if col_name and col_name in combined.columns:
            val = combined[col_name].dropna().iloc[0] if len(combined[col_name].dropna()) > 0 else None
            if val is not None:
                return float(val)
        return None

    specs = {
        'usl': _extract_spec(mapping_config.get('usl')),
        'lsl': _extract_spec(mapping_config.get('lsl')),
        'ref_upper': _extract_spec(mapping_config.get('ref_upper')),
        'ref_lower': _extract_spec(mapping_config.get('ref_lower'))
    }
...
def process_capability(df, specs, subgroup_stats, chart_type):
    ...
    if chart_type == 'X-R':
        ...
        S_bar = np.sqrt(pooled_var)
        dof = (ni - 1).sum() + 1
        c4_val = get_constants(dof)['c4'] if dof in get_constants(0) else (1 - 1/(4*dof))
